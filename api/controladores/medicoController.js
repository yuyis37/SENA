...
var medicoModel = require(appRoot + "/api/modelos/medicoModel.js").medico;

var medicoController = {};

medicoController.guardar = function(request,response){
  var post = {
    id:request.body.id,
  ...
  medicoModel.guardar(post,function (respuesta) {
...
medicoController.listar = function (request, response) {
    medicoModel.listar(null,function (respuesta) {
      console.log("--------------->response");
      console.log(respuesta);
...
medicoController.listarId = function(request,response){
  var post={
    id:request.body.id
  }
 ...
 medicoModel.listarId(post,function(respuesta){
   response.json(respuesta)
 }) 
}

medicoController.actualizar = function(request,response){
  var producto = {
      nombre:request.body.nombre,
      identificacion:request.body.identificacion,
      email:request.body.email,
  ...
  medicoModel.actualizar(producto,function(respuesta){
      console.log('------->response')
      console.log(respuesta)
      response.json(respuesta)
  })
}  
medicoController.eliminar = function (request, response) {
  var post = {
    id:request.body.id
  }
  if (
   ...
   medicoModel.eliminar(post,function(resultado){
     response.json(resultado)
   })
 };


    module.exports.medico = medicoController;
